...
class U2FDeviceForm(DeviceValidationForm):
    token = forms.CharField(label=_("Token"), widget=forms.PasswordInput())
    ...
    def save(self):
        ...
        if len(self.request.user.u2f_keys.all()) == 0:
            ...
            name = "key"
        return U2FDevice.objects.create(name=name, public_key=self.u2f_device['publicKey'],
                                        key_handle=self.u2f_device['keyHandle'], app_id=self.u2f_device['appId'],
                                        user=self.user)
