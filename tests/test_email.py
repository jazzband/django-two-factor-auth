...
class EmailTest(UserMixin, TestCase):
    ...
    def test_email_backend_not_set(self):
        msg = "Please configure the EMAIL_BACKEND and DEFAULT_FROM_EMAIL settings to use the Email plugin."
        with self.assertRaisesMessage(ImproperlyConfigured, msg):
            apps.get_app_config('email').ready()

    @override_settings(DEFAULT_FROM_EMAIL=None)
    def test_default_from_email_not_set(self):
        msg = "Please configure the EMAIL_BACKEND and DEFAULT_FROM_EMAIL settings to use the Email plugin."
        with self.assertRaisesMessage(ImproperlyConfigured, msg):
            apps.get_app_config('email').ready()
    ...
    def test_setup(self, signals):
        ...
        self.assertEqual(msg.subject, 'Authentication token email')
    ...
    def test_subject_and_text(self):
        self.user.emaildevice_set.create(name='default', key=random_hex())
